...
func main() {
	...
	headless := flag.Bool("headless", false, "run without GUI window")
	flag.Parse()

	// 若默认 config.json 不存在，自动创建默认配置文件供用户查阅与修改
	...
	if strings.Contains(cfg.Listen, ":") {
		parts := strings.Split(cfg.Listen, ":")
	...
	uiURL := fmt.Sprintf("http://%s:%s", host, port)
	...
	// 桌面窗口模式 (非 headless)
	if !*headless {
		if runDesktopGUI(uiURL, stop) {
			return
		}
	}

	// 无头模式或不支持 webview 时的回退
	if !*headless {
		go func() {
			time.Sleep(500 * time.Millisecond)
			log.Printf("[WebUI] 正在打开控制面板: %s", uiURL)
			openBrowser(uiURL)
		}()
	}
